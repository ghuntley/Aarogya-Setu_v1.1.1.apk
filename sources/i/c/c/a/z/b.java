package i.c.c.a.z;

import com.google.protobuf.CodedOutputStream;
import com.google.protobuf.InvalidProtocolBufferException;
import i.c.c.a.z.l;
import i.c.c.a.z.r1;
import i.c.f.g;
import i.c.f.i;
import i.c.f.k;
import i.c.f.o;
import i.c.f.q;
import java.io.IOException;

/* compiled from: AesCtrHmacAeadKeyFormat */
public final class b extends k<b, C0129b> implements c {

    /* renamed from: j  reason: collision with root package name */
    public static final b f4396j;

    /* renamed from: k  reason: collision with root package name */
    public static volatile q<b> f4397k;

    /* renamed from: h  reason: collision with root package name */
    public l f4398h;

    /* renamed from: i  reason: collision with root package name */
    public r1 f4399i;

    /* compiled from: AesCtrHmacAeadKeyFormat */
    public static /* synthetic */ class a {
        public static final /* synthetic */ int[] a;

        /* JADX WARNING: Can't wrap try/catch for region: R(20:0|(2:1|2)|3|5|6|7|8|9|11|12|13|14|15|(2:17|18)|19|21|22|23|24|26) */
        /* JADX WARNING: Can't wrap try/catch for region: R(22:0|1|2|3|5|6|7|8|9|11|12|13|14|15|17|18|19|21|22|23|24|26) */
        /* JADX WARNING: Code restructure failed: missing block: B:27:?, code lost:
            return;
         */
        /* JADX WARNING: Failed to process nested try/catch */
        /* JADX WARNING: Missing exception handler attribute for start block: B:13:0x0025 */
        /* JADX WARNING: Missing exception handler attribute for start block: B:23:0x0039 */
        /* JADX WARNING: Missing exception handler attribute for start block: B:7:0x0017 */
        static {
            /*
                i.c.f.k$j[] r0 = i.c.f.k.j.values()
                int r0 = r0.length
                int[] r0 = new int[r0]
                a = r0
                r1 = 1
                r2 = 4
                i.c.f.k$j r3 = i.c.f.k.j.NEW_MUTABLE_INSTANCE     // Catch:{ NoSuchFieldError -> 0x000f }
                r0[r2] = r1     // Catch:{ NoSuchFieldError -> 0x000f }
            L_0x000f:
                r0 = 2
                int[] r3 = a     // Catch:{ NoSuchFieldError -> 0x0017 }
                i.c.f.k$j r4 = i.c.f.k.j.IS_INITIALIZED     // Catch:{ NoSuchFieldError -> 0x0017 }
                r4 = 0
                r3[r4] = r0     // Catch:{ NoSuchFieldError -> 0x0017 }
            L_0x0017:
                int[] r3 = a     // Catch:{ NoSuchFieldError -> 0x001e }
                i.c.f.k$j r4 = i.c.f.k.j.MAKE_IMMUTABLE     // Catch:{ NoSuchFieldError -> 0x001e }
                r4 = 3
                r3[r4] = r4     // Catch:{ NoSuchFieldError -> 0x001e }
            L_0x001e:
                r3 = 5
                int[] r4 = a     // Catch:{ NoSuchFieldError -> 0x0025 }
                i.c.f.k$j r5 = i.c.f.k.j.NEW_BUILDER     // Catch:{ NoSuchFieldError -> 0x0025 }
                r4[r3] = r2     // Catch:{ NoSuchFieldError -> 0x0025 }
            L_0x0025:
                int[] r2 = a     // Catch:{ NoSuchFieldError -> 0x002b }
                i.c.f.k$j r4 = i.c.f.k.j.VISIT     // Catch:{ NoSuchFieldError -> 0x002b }
                r2[r1] = r3     // Catch:{ NoSuchFieldError -> 0x002b }
            L_0x002b:
                r1 = 6
                int[] r2 = a     // Catch:{ NoSuchFieldError -> 0x0032 }
                i.c.f.k$j r3 = i.c.f.k.j.MERGE_FROM_STREAM     // Catch:{ NoSuchFieldError -> 0x0032 }
                r2[r0] = r1     // Catch:{ NoSuchFieldError -> 0x0032 }
            L_0x0032:
                r0 = 7
                int[] r2 = a     // Catch:{ NoSuchFieldError -> 0x0039 }
                i.c.f.k$j r3 = i.c.f.k.j.GET_DEFAULT_INSTANCE     // Catch:{ NoSuchFieldError -> 0x0039 }
                r2[r1] = r0     // Catch:{ NoSuchFieldError -> 0x0039 }
            L_0x0039:
                int[] r1 = a     // Catch:{ NoSuchFieldError -> 0x0041 }
                i.c.f.k$j r2 = i.c.f.k.j.GET_PARSER     // Catch:{ NoSuchFieldError -> 0x0041 }
                r2 = 8
                r1[r0] = r2     // Catch:{ NoSuchFieldError -> 0x0041 }
            L_0x0041:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: i.c.c.a.z.b.a.<clinit>():void");
        }
    }

    static {
        b bVar = new b();
        f4396j = bVar;
        bVar.f();
    }

    public static /* synthetic */ void a(b bVar, l lVar) {
        if (lVar != null) {
            bVar.f4398h = lVar;
            return;
        }
        throw null;
    }

    public int c() {
        int i2 = this.f4950g;
        if (i2 != -1) {
            return i2;
        }
        int i3 = 0;
        if (this.f4398h != null) {
            i3 = 0 + CodedOutputStream.b(1, (o) i());
        }
        if (this.f4399i != null) {
            i3 += CodedOutputStream.b(2, (o) j());
        }
        this.f4950g = i3;
        return i3;
    }

    public l i() {
        l lVar = this.f4398h;
        return lVar == null ? l.f4499j : lVar;
    }

    public r1 j() {
        r1 r1Var = this.f4399i;
        return r1Var == null ? r1.f4575j : r1Var;
    }

    /* renamed from: i.c.c.a.z.b$b  reason: collision with other inner class name */
    /* compiled from: AesCtrHmacAeadKeyFormat */
    public static final class C0129b extends k.b<b, C0129b> implements c {
        public C0129b() {
            super(b.f4396j);
        }

        public /* synthetic */ C0129b(a aVar) {
            super(b.f4396j);
        }
    }

    public static /* synthetic */ void a(b bVar, r1 r1Var) {
        if (r1Var != null) {
            bVar.f4399i = r1Var;
            return;
        }
        throw null;
    }

    public void a(CodedOutputStream codedOutputStream) {
        if (this.f4398h != null) {
            codedOutputStream.a(1, (o) i());
        }
        if (this.f4399i != null) {
            codedOutputStream.a(2, (o) j());
        }
    }

    public final Object a(k.j jVar, Object obj, Object obj2) {
        switch (jVar.ordinal()) {
            case 0:
                return f4396j;
            case 1:
                k.C0142k kVar = (k.C0142k) obj;
                b bVar = (b) obj2;
                this.f4398h = (l) kVar.a(this.f4398h, bVar.f4398h);
                this.f4399i = (r1) kVar.a(this.f4399i, bVar.f4399i);
                return this;
            case 2:
                g gVar = (g) obj;
                i iVar = (i) obj2;
                boolean z = false;
                while (!z) {
                    try {
                        int i2 = gVar.i();
                        if (i2 != 0) {
                            if (i2 == 10) {
                                l.b bVar2 = this.f4398h != null ? (l.b) this.f4398h.e() : null;
                                l lVar = (l) gVar.a(l.f4499j.g(), iVar);
                                this.f4398h = lVar;
                                if (bVar2 != null) {
                                    bVar2.a(lVar);
                                    this.f4398h = (l) bVar2.j();
                                }
                            } else if (i2 == 18) {
                                r1.b bVar3 = this.f4399i != null ? (r1.b) this.f4399i.e() : null;
                                r1 r1Var = (r1) gVar.a(r1.f4575j.g(), iVar);
                                this.f4399i = r1Var;
                                if (bVar3 != null) {
                                    bVar3.a(r1Var);
                                    this.f4399i = (r1) bVar3.j();
                                }
                            } else if (!gVar.e(i2)) {
                            }
                        }
                        z = true;
                    } catch (InvalidProtocolBufferException e) {
                        throw new RuntimeException(e);
                    } catch (IOException e2) {
                        throw new RuntimeException(new InvalidProtocolBufferException(e2.getMessage()));
                    }
                }
                break;
            case 3:
                return null;
            case 4:
                return new b();
            case 5:
                return new C0129b((a) null);
            case 6:
                break;
            case 7:
                if (f4397k == null) {
                    synchronized (b.class) {
                        if (f4397k == null) {
                            f4397k = new k.c(f4396j);
                        }
                    }
                }
                return f4397k;
            default:
                throw new UnsupportedOperationException();
        }
        return f4396j;
    }
}
